...
<div class="container">
    ...
    <?php
        ...
        $conn = mysqli_connect($dbservername ,$dbusername, $dapassword,$dbname);
           ...
           $sql="SELECT *FROM cars where price <= '$price' and cartype='$type'";
           $result=$conn->query($sql);
        ...
        if ($result->num_rows > 0) {
          echo'<h2>Avalible cars</h2>';
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<div class="card">';
                echo '<img src="./cars/'. $row["img"].'"  class="card-img-top" alt="' . $row["cartype"] . '">';
                echo '<div class="card-body">';
                echo '<h5 class="card-title">' . $row["airport"] . '</h5>';
                echo '<h3 class="card-text">' . $row["carNmae"] . '</h3>';
